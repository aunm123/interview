...
const COMMON_ATTS = ['fill', 'fillOpacity', 'stroke', 'strokeWidth', 'strokeOpacity', 'opacity',
	'strokeLinecap', 'strokeLinejoin', 'transform', 'strokeDasharray', 'strokeDashoffset',
	...
	obtainComponentAtts({attributes}, enabledAttributes) {
		...
		const componentAtts = Array.from(attributes)
			...
			.reduce((acc, {nodeName, nodeValue}) => {
				if (this.state.fill && nodeValue == '#4A90E2') {
					acc[nodeName] = this.state.fill;
				} else {
					acc[nodeName] = nodeValue;
				}
				// if (this.state.fill && nodeValue !== 'none') {
				// 	if ( nodeValue == '#4A90E2') {
				// 		acc[nodeName] = this.state.fill;
				// 	}
				// 	if (nodeName == 'stroke' && nodeValue == '#4A90E2') {
				// 		// #4A90E2
				// 		acc[nodeName] = this.state.fill;
				// 	}
				// } else {
