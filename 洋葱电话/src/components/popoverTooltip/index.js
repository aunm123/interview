...
class PopoverTooltip extends React.PureComponent<Props, State> {
	...
	onInnerContainerLayout = (
		...
		componentWrapper.measure((x, y, width, height, pageX, pageY) => {
			...
			let tooltipContainerX_final = pageX + (width - tooltipContainerWidth) / 2;
			...
			console.log("tooltipContainerHeight: " + tooltipContainerHeight);

			console.log("tooltipContainerX_final: " + tooltipContainerX_final);
			console.log("tooltipContainerY_final: " + tooltipContainerY_final);
			console.log("tooltipTriangleDown: " + tooltipTriangleDown);

			// 验证顶部是否有足够位置显示菜单列表 (iphone X 头部 34 - 边框 10)
			if (pageY - tooltipContainerHeight - 24 < 0) {
				// 列表在下面
				tooltipContainerY_final = pageY + height + 10;
				tooltipTriangleDown = false;
			}
			// 验证底部是否有足够位置显示菜单列表
			if (pageY + tooltipContainerHeight + height + 10 > window.height) {
				// 列表在上面
			...
			// 验证右边是否有足够位置显示菜单列表
			if (tooltipContainerX_final + tooltipContainerWidth > window.width) {
				// 右边位置不足够
				tooltipContainerX_final = window.width - tooltipContainerWidth;
			}
						...
						>
							...
							>
								<View
									...
									style={[styles.innerContainer, {zIndex: 99}]}
								>
									{triangleUp}
									<View style={[
