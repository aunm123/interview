...
@inject('store', 'global')
@observer
export default class AddWhiteListPage extends BaseComponents {
	...
	constructor(props) {
		...
		this.isAll = true;
		this.whiteList = this.navigation.getParam('whiteList');
	...
	search() {
		...
		for (let item of dataList) {
			...
			let needAdd = true;
			for (let jtem of this.whiteList) {
	...
	async addWhiteList(item) {
		...
		try {
			...
			// 添加白名单
			await Req.post(URLS.ADD_WHITE_LIST, { content: blackListContent });
			// 移除黑名单
			await this.configService.insertOrSaveConfig(contract, null, 0);
