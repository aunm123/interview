...
@inject('store', 'global')
@observer
class BlackListPage extends BaseComponents {
	...
	async actionSheetShow(item) {
		...
		this.props.showActionSheetWithOptions({
			...
			buttonIndex => {
				switch (buttonIndex) {
					case 0: {
						Alert.alert('确定需要把该联系人移出黑名单？', '', [{
							text: '确定', onPress: async () => {
								this.global.showLoading();
								await this.configService.insertOrSaveConfig(item, null, 0);
								await this.init();
					...
					<NavBar title={'黑名单'}
							...
							rightRender={(
								<Button style={{paddingRight: 6}} onPress={()=>{
									this.navigation.push("AddBlackListPage", {blackList: this.blackList})
								}}>
...
export default connectActionSheet(BlackListPage)
