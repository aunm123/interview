...
@inject('store', 'global')
@observer
export default class CallComingPage extends BaseComponents {
	...
	@observable
	country_no = this.global.userData.caller_country_no;
	@observable
	phone_no = this.global.userData.caller_phone_no;
	@observable
	callComingSwitch = this.global.userData.caller_status;
	...
	constructor(props) {
		...
		this.mode = this.callComingSwitch ? (this.global.userData.caller_country_no ? 0 : 1) : 2;
	}
			...
			<View style={{justifyContent: 'center', alignItems: 'center'}}>
				...
				<View style={{
					...
					<TextEx style={{
						...
						{'+' + this.global.userData.caller_country_no + " " + this.global.userData.caller_phone_no}
					</TextEx>
					<Button onPress={()=>{
						this.mode = 1
					}}>
	...
	async nextBtnPress() {
		...
		try {
			let code = await this.otherService.getCallComingVerifyCode(this.country_no, this.phone_no);
			this.global.dismissLoading();

			let phone = "+" + this.country_no + " " + this.phone_no;
			code = code.split("").join(" ");
			this.navigation.push('CallComingStep1Page', {phone: phone, code: code})
		} catch (e) {
			this.global.dismissLoading();
		}
	}
		...
		return (
			<View style={{justifyContent: 'center', alignItems: 'center'}}>
				<TextEx style={{
					...
					输入从洋葱onioncall拨打手机和座机或发送短信时希望显示的电话号码
				</TextEx>
							...
							rightRender={(
								<View style={{marginRight: 8, marginBottom: 5}}>
									<Switch
										...
										value={this.callComingSwitch == 1}
										onSyncPress={(value) => {
											this.callComingSwitch = value;
											try {
												this.otherService.callComingSwitch(value ? "1" : "0");
												this.global.userData.caller_status = value ? 1 : 0;
												this.mode = value ? (this.global.userData.caller_country_no ? 0 : 1) : 2;
											} catch (e) {
												this.callComingSwitch = !value;
											}
										}}
